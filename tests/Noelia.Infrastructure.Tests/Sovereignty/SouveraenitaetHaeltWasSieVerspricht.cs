...
public class SouveraenitaetHaeltWasSieVerspricht
{
    ...
    /// failed to load. Thirteen log statements in Noelia itself say
    /// <c>{SecretName}</c>.
    /// </summary>
    [Theory]
    ...
    /// <summary>
    /// Everything Noelia sets up appears in the composition, so a service can
    ...
    public void Die_souveraene_Plattform_steht_in_der_Zusammensetzung()
    {
        ...
        zusammensetzung.Included.Should().Contain(NoeliaModule.SovereignPlatform);
    }
    ...
    public void Sie_laesst_sich_mit_Grund_abwaehlen()
    {
        var zusammensetzung = Zusammensetzen(noelia => noelia
            .UseDefaults()
            .AddSovereignPlatform(s => s.Allow("openbao.internal"))
            .Without(NoeliaModule.SovereignPlatform, "Testumgebung ruft absichtlich nach draußen"));

        zusammensetzung.Included.Should().NotContain(NoeliaModule.SovereignPlatform);
        zusammensetzung.Excluded[NoeliaModule.SovereignPlatform].Should().Contain("Testumgebung");
    }
    ...
    {
        var dienste = Dienste(noelia => noelia
            .UseDefaults()
            .AddSovereignPlatform(s => s.Allow("openbao.internal"))
            .Without(NoeliaModule.SovereignPlatform, "Testumgebung"));
    ...
    {
        var dienste = Dienste(noelia => noelia.AddSovereignPlatform(konfigurieren));
    ...
    private static NoeliaComposition Zusammensetzen(Action<NoeliaBuilder> konfigurieren) =>
        Dienste(konfigurieren).BuildServiceProvider().GetRequiredService<NoeliaComposition>();
    ...
    private static IServiceCollection Dienste(Action<NoeliaBuilder> konfigurieren)
    {
        ...
        dienste.AddNoelia(Konfiguration(), Umgebung(), "test-service", konfigurieren);
        return dienste;
    }
