...
using Noelia.Application.Behaviors;
using Noelia.Application.Extensions;
using Noelia.Fixtures.CachingCqrs;
using Noelia.Fixtures.PlainCqrs;
...
namespace Noelia.Infrastructure.Tests.Cqrs;
...
public class CqrsCacheRequirementTests
{
    ...
    public void Die_Meldung_nennt_den_Typ_der_die_Anforderung_ausloest()
    {
        ...
        ConfigurePipeline(app).Should().Throw<InvalidOperationException>()
            .WithMessage($"*{nameof(GetJobQuery)}*{nameof(Noelia.Application.Interfaces.ICacheableQuery)}*");
    }
