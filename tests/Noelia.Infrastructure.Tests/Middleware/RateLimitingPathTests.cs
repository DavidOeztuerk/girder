...
namespace Noelia.Infrastructure.Tests.Middleware;
...
public class RateLimitingPathTests
{
    ...
    private static DistributedRateLimitingMiddleware CreateMiddlewareInstance()
    {
        ...
        var rateLimitStore = Substitute.For<Noelia.Abstractions.Caching.IDistributedRateLimitStore>();
        var logger = Substitute.For<Microsoft.Extensions.Logging.ILogger<DistributedRateLimitingMiddleware>>();
        var options = Microsoft.Extensions.Options.Options.Create(new Noelia.Infrastructure.Models.DistributedRateLimitingOptions());
