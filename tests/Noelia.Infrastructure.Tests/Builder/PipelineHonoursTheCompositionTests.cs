...
/// startup or forced the caller to write Noelia's chain out themselves, minus
/// the lines they wanted gone. That copy is wrong the first time Noelia adds a
...
public class PipelineHonoursTheCompositionTests
{
    ...
    public async Task The_defaults_and_the_default_chain_start_a_service()
    {
        using var host = await Start(
            ...
            (app, env) => app.UseNoelia(env, "test-service"));
    ...
    public async Task A_module_left_out_does_not_break_the_chain()
    {
        using var host = await Start(
            noelia => noelia
                .UseDefaults()
                .Without(NoeliaModule.RateLimiting, "braking happens at the entrance"),
            (app, env) => app.UseNoelia(env, "test-service"));
    ...
    public async Task Writing_the_step_out_by_hand_does_not_undo_the_exclusion()
    {
        using var host = await Start(
            noelia => noelia
                .UseDefaults()
                .Without(NoeliaModule.RateLimiting, "braking happens at the entrance"),
            (app, env) => app.UseNoelia(env, "test-service", pipeline => pipeline
                .UseRateLimiting()
    ...
    public async Task Without_permission_enforcement_an_anonymous_call_gets_through()
    {
        using var host = await Start(
            noelia => noelia
                .UseDefaults()
                .Without(NoeliaModule.PermissionEnforcement, "we have a public surface"),
            (app, env) => app.UseNoelia(env, "test-service"));
    ...
    public async Task The_policy_provider_stays_when_only_the_middleware_goes()
    {
        ...
        var composition = host.Services.GetRequiredService<NoeliaComposition>();

        composition.Included.Should().Contain(NoeliaModule.Authorization);
        composition.Excluded.Should().ContainKey(NoeliaModule.PermissionEnforcement);
    }
    ...
    public async Task A_step_whose_module_is_not_a_default_does_not_demand_its_provider()
    {
        ...
        host.Services.GetService<Noelia.Infrastructure.Caching.Http.ICachePolicyProvider>()
                ...
                .ConfigureServices((context, services) =>
                    services.AddNoelia(
                        ...
                        noelia =>
                        {
                            ...
                            // service has it.
                            noelia.UseJwt(jwt => jwt.FromSharedSecret());
                        }))
