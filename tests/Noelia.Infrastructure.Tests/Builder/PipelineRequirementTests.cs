...
/// failure used to arrive in production as a 500 naming a Noelia-internal type
...
public class PipelineRequirementTests
{
    ...
    public void A_step_without_requirements_composes_alone(string step)
    {
        ...
        var compose = () => app.UseNoelia(app.Environment, "test", pipeline =>
        {
            switch (step)
            {
