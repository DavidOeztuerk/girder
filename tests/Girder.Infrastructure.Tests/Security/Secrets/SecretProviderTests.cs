...
public class AwsSecretsManagerProviderTests
{
    private static AwsSecretsManagerProvider CreateProvider()
    {
        ...
        var config = new ConfigurationBuilder()
            ...
            .Build();
        return new AwsSecretsManagerProvider(logger, config);
    }

    [Fact]
    public void Constructor_WithRegionConfig_DoesNotThrow()
    {
    ...
    public void Constructor_WithEmptyConfig_DoesNotThrow()
    {
        ...
        var act = () => new AwsSecretsManagerProvider(logger, config);

        act.Should().NotThrow();
    }

    [Fact]
    public async Task GetSecretAsync_WithoutCredentials_ReturnsNullGracefully()
    {
        // Without real AWS credentials the provider should return null rather than propagate exceptions
        var provider = CreateProvider();

        var result = await provider.GetSecretAsync("some-secret");

        result.Should().BeNull();
    }

    [Fact]
    public async Task SecretExistsAsync_WithoutCredentials_ReturnsFalseGracefully()
    {
        var provider = CreateProvider();

        var result = await provider.SecretExistsAsync("some-secret");

        result.Should().BeFalse();
    }

    [Fact]
    public async Task SetSecretAsync_WithoutCredentials_DoesNotPropagateException()
    {
        var provider = CreateProvider();

        var act = async () => await provider.SetSecretAsync("key", "value");

        await act.Should().NotThrowAsync();
    }

    [Fact]
    public async Task DeleteSecretAsync_WithoutCredentials_DoesNotPropagateException()
    {
        var provider = CreateProvider();
    ...
    [Fact]
    public async Task ListSecretKeysAsync_WithoutCredentials_DoesNotPropagateException()
    {
        var provider = CreateProvider();

        var act = async () => await provider.ListSecretKeysAsync();
...
{
    private static AzureKeyVaultProvider CreateProvider()
    {
        var logger = Substitute.For<ILogger>();
        var config = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
                ["Azure:KeyVault:Uri"] = "https://myvaultxyz.vault.azure.net"
            })
            .Build();
        return new AzureKeyVaultProvider(logger, config);
    }

    [Fact]
    public void Constructor_WithVaultUriConfig_DoesNotThrow()
    {
        var act = () => CreateProvider();
    ...
    public void Constructor_WithEmptyConfig_DoesNotThrow()
    {
        ...
        var act = () => new AzureKeyVaultProvider(logger, config);
    ...
    public async Task GetSecretAsync_WithoutCredentials_ReturnsNullGracefully()
    {
        // Without managed identity or SP credentials, provider returns null rather than throwing
        var provider = CreateProvider();
    ...
    [Fact]
    public async Task GetSecretVersionAsync_WithoutCredentials_DoesNotPropagateException()
    {
        var provider = CreateProvider();

        var act = async () => await provider.GetSecretVersionAsync("key", "1");

        await act.Should().NotThrowAsync();
    }

    [Fact]
    public async Task ListSecretVersionsAsync_WithoutCredentials_DoesNotPropagateException()
    {
        var provider = CreateProvider();

        var act = async () => await provider.ListSecretVersionsAsync("key");
