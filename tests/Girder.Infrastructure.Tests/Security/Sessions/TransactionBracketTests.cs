...
public class TransactionBracketTests : IDisposable
{
    ...
    private async Task<bool> ExistsAsync(string token)
    {
        ...
        return await reader.Set<GirderRefreshToken>().AsNoTracking()
    ...
    private sealed class RaceInterceptor : DbCommandInterceptor
    {
            ...
            CancellationToken cancellationToken = default)
        {
            if (_hash is not { } hash
                ...
                || !command.CommandText.Contains("girder_refresh_tokens", StringComparison.Ordinal))
            {
            ...
            winner.CommandText =
                """UPDATE girder_refresh_tokens SET "ReplacedBy" = "Id" WHERE "TokenHash" = $hash""";
