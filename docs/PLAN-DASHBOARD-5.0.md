...
Noelia weiß schon fast alles über sich selbst — es kann es nur niemandem zeigen.

| Was Noelia weiß | Wo es liegt | Wer es heute sieht |
|---|---|---|
| welche Module laufen, und welche warum nicht | `NoeliaComposition` | niemand, außer man protokolliert es selbst |
...
abruft. Das ist die Lücke: **die Zusammensetzung ist der Kern von Noelia 4, und
sie ist unsichtbar.** Das Dashboard ist die sichtbare Form davon.

Der zweite Grund ist die Aufnahme. Wer Noelia das erste Mal einrichtet, hat heute
...
`/health`. Noelias Zusage — *„adding Noelia never adds a server you have to
run"* — gilt hier genauso.

**Kein Fremdpaket.** `Noelia.Infrastructure` zieht 44 transitive Pakete; das war
der Grund für `Noelia.Http`. Ein Dashboard, das Blazor, ein npm-Bündel oder ein
CDN mitbrächte, verlöre genau das wieder. Server-gerendertes HTML, eingebettete
Ressourcen, etwas Vanilla-JS. Messlatte wie bei `Noelia.Http`: **null**.
...
`Noelia.Dashboard` — eigenes Paket, `FrameworkReference` auf
`Microsoft.AspNetCore.App`, Projektverweis auf `Noelia.Abstractions`, sonst
...
`NoeliaModule.Dashboard` — **nicht** in `UseDefaults()`. Es zeigt, wohin ein
...
| Fläche | Quelle | Zeigt |
|---|---|---|
| **Zusammensetzung** | `NoeliaComposition`, `ProviderRequirements` | jedes Modul: in Betrieb, abgewählt mit Grund, oder nicht genannt. Fehlende Anbieter mit dem Aufruf, der sie behebt |
...
|---|---|---|
| 1 | Paket + Wächter | `Noelia.Dashboard` steht, Fremdpaketzahl **0**, Test dagegen |
...
1. **Ist das Dashboard ein Modul oder ein eigenes Paket, das Module liest?**
   Der Entwurf oben sagt: beides — Paket `Noelia.Dashboard`, Modul
   `NoeliaModule.Dashboard`. Das kostet ein Paket mehr im Verzeichnis und ist
   dafür abwählbar wie alles andere.

2. **Wie kommt es an `NoeliaComposition`, ohne `Noelia.Infrastructure` zu
   referenzieren?** `NoeliaComposition` liegt in `Noelia.Abstractions` — das
   trägt. `ISovereigntyReport` und `IAuditTrailService` liegen dagegen in
   `Noelia.Infrastructure`. Entweder wandern ihre Schnittstellen nach
