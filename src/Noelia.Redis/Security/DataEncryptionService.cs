...
namespace Noelia.Redis.Security.Encryption;
...
public class DataEncryptionService : IDataEncryptionService
{
    ...
    /// <summary>
    /// The 4.4.1 predecessor encrypted the payload, but authenticated only caller AAD;
        ...
        try
        {
            ...
            var encryptionResult = options.Algorithm switch
            {
                ...
                _ => throw new NotSupportedException(
                    $"{options.Algorithm} is not implemented. Noelia ships AES256GCM and "
            ...
            hash = options.Algorithm switch
            {
                ...
                HashingAlgorithm.Argon2id or HashingAlgorithm.Argon2i or HashingAlgorithm.Argon2d
                    ...
                    throw new NotSupportedException(
                        $"{options.Algorithm} is not implemented. Noelia ships no memory-hard "
    ...
    /// that Noelia ships no encryption. The first was chosen because the
    ...
    private async Task<byte[]> DecryptAesGcmAsync(EncryptedDataInfo encryptedInfo, EncryptionKey key)
    {
        ...
        if (encryptedInfo.Version == PreFixEnvelopeVersion)
        {
            throw new CryptographicException(
                "This envelope was written by the 4.4.0 predecessor or earlier, whose Data field holds "
                + "the PLAINTEXT Base64-encoded and whose AuthTag is all zeroes — it was never "
                + "encrypted (advisory: Noelia.Redis, fixed in 4.4.1). It is refused rather "
        ...
        {
            throw new CryptographicException(
                "This envelope was written by the 4.4.1 predecessor. Its payload is encrypted, but "
        ...
        IReadOnlyDictionary<string, string> metadata)
    {
        ...
        // Protocol domain, not branding. Changing it would invalidate every
        // authenticated 4.4.2+ predecessor envelope already stored by consumers.
        WriteRequired(writer, "Noelia.Redis.EncryptionEnvelope");
