    ...
    private static byte[] DeriveP256(string seed)
    {
        for (var counter = 0; counter < 1000; counter++)
        {
            var candidate = SHA256.HashData(
                System.Text.Encoding.UTF8.GetBytes($"noelia-dev-key:{counter}:{seed}"));
...
public sealed record GeneratedKeyPair(string Kid, string PrivateKey, string PublicKey)
{
    ...
    public string ToEnvironmentLines() =>
        $"NOELIA_JWT_KID={Kid}{Environment.NewLine}"
        + $"NOELIA_JWT_PRIVATE_KEY={PrivateKey}{Environment.NewLine}"
        + $"NOELIA_JWT_PUBLIC_KEY={PublicKey}";
}
