...
public sealed class RateLimitingBuilder
{
    ...
    internal RateLimitingBuilder(NoeliaBuilder noelia) => _noelia = noelia;
    ...
    public RateLimitingBuilder PerSubject(Func<HttpContext, string> subject)
    {
        ...
        _noelia.Services.Configure<DistributedRateLimitingOptions>(options =>
        {
    ...
    public RateLimitingBuilder TrustForwardedHeadersFrom(params string[] proxies)
    {
        ...
        _noelia.Services.TrustForwardedHeadersFrom(proxies);
        _trustDeclared = true;
        return this;
    }
...
public static class RateLimitingModuleExtensions
{
    ...
    /// rather than beside a <c>Use(NoeliaModule.RateLimiting)</c> that would say
    /// the same thing twice.
    /// </remarks>
    public static NoeliaBuilder UseRateLimiting(
        ...
        Action<RateLimitingBuilder> configure)
    {
        ...
        noelia.Use(NoeliaModule.RateLimiting);
        configure(new RateLimitingBuilder(noelia));
        return noelia;
    }
}
