...
/// <summary>
/// Chooses what Noelia sets up for this service.
/// </summary>
/// <remarks>
...
/// <para>
/// <see cref="Use(NoeliaModule)"/> adds and
/// <see cref="Without(NoeliaModule, string)"/> removes, in any order, and the
/// last mention of a module wins. The order modules are registered in is
/// Noelia's, not the order they were named: modules depend on one another, and a
...
public sealed class NoeliaBuilder
{
    ...
    /// <summary>
    /// Sets up a module Noelia knows.
    /// </summary>
    /// <remarks>
    /// Undoes a previous <see cref="Without(NoeliaModule, string)"/> for the
    ...
    public NoeliaBuilder Use(NoeliaModule module)
    {
        if (!_catalogue.ContainsKey(module))
        {
            throw new ArgumentException(
                $"Noelia has no module called '{module}'. A module from another package "
    ...
    /// <summary>
    /// Sets up a module from outside Noelia.
    /// </summary>
    /// <remarks>
    ...
    /// run; Noelia needs to know nothing about it, and the module then behaves
    ...
    /// to what it explains.
    /// <para>
    /// Naming a module Noelia does not know is allowed: a package may be removed
    ...
    /// <param name="reason">Why. Recorded in <see cref="NoeliaComposition"/>.</param>
